...
require_once(__DIR__ . '/api/attach_and_extract.php');
require_once(__DIR__ . '/citations/CitationContract.php');
require_once(__DIR__ . '/citations/ClaimCitationMapper.php');
require_once(__DIR__ . '/citations/CitationSourceResolver.php');
require_once(__DIR__ . '/agents/CitationValidationWorker.php');
require_once(__DIR__ . '/agents/AgentTrace.php');
require_once(__DIR__ . '/agents/GuidelineCorpusWorker.php');
require_once(__DIR__ . '/agents/SparseRetrievalWorker.php');
require_once(__DIR__ . '/agents/DenseRetrievalWorker.php');
require_once(__DIR__ . '/agents/HybridRetrievalWorker.php');
require_once(__DIR__ . '/agents/RerankWorker.php');
...
if ($action === 'attach_and_extract_lab_pdf') {
    ...
    if ($permissionResponse !== []) {
        ...
        $meta = aiCopilotAugmentMetaWithDocumentIngestion($meta, $context);
        $permissionResponsePayload = [
            'answer' => $permissionResponse['answer'],
            'sections' => $permissionResponse['sections'],
            'tags' => $permissionResponse['tags'],
        ];
        $citationContract = aiCopilotBuildCitationContractPayload($permissionResponsePayload, $context, $role, $mode, $message, $requestId);
        $meta = aiCopilotAugmentMetaWithCitationContract($meta, $citationContract);
        aiCopilotJsonResponse(200, [
            'ok' => true,
            'mode' => $mode,
            'role' => $role,
            'patient' => !empty($context['patient']['name']) ? $context['patient']['name'] : null,
            'answer' => $citationContract['response']['answer'],
            'sections' => $citationContract['response']['sections'],
            'tags' => $citationContract['response']['tags'],
            'sources' => $sources,
            'claims' => $citationContract['claims'],
            'sources_used' => $citationContract['sources_used'],
            'uncited_claims_blocked' => $citationContract['uncited_claims_blocked'],
            'safety_status' => $citationContract['safety_status'],
    ...
    $citationContract = aiCopilotBuildCitationContractPayload($draft, $context, $role, $mode, $message, $requestId);
    $draft = $citationContract['response'];
    $meta = aiCopilotBuildResponseMeta(
        $requestId,
        $role,
        $mode,
...
if ($action === 'clear_lab_evidence') {
    if (empty($patient)) {
        aiCopilotJsonResponse(400, [
            'error' => 'Select a demo patient before clearing uploaded lab evidence.',
            'meta' => aiCopilotErrorMeta($requestId, 'patient_required_for_clear'),
        ]);
        exit;
    }

    $patientKey = aiCopilotCleanText((string) ($patient['pubpid'] ?? ''));
    $clearCounts = aiCopilotClearUploadedLabEvidenceForPatient($patientKey, $session);
    aiCopilotJsonResponse(200, [
        'ok' => true,
        'message' => 'Uploaded lab evidence cleared.',
        'patient' => aiCopilotCleanText(trim(((string) ($patient['fname'] ?? '')) . ' ' . ((string) ($patient['lname'] ?? '')))),
        'selected_patient_key' => $patientKey,
        'counts' => $clearCounts,
        'meta' => array_merge(
            aiCopilotErrorMeta($requestId, 'clear_lab_evidence'),
            [
...
function aiCopilotAugmentMetaWithDocumentIngestion(array $meta, array $context): array
{
    $documentIngestionResult = is_array($context['document_ingestion_result'] ?? null) ? $context['document_ingestion_result'] : [];
    if ($documentIngestionResult === []) {
        return $meta;
    }

    $workflowTrace = array_values(array_filter($documentIngestionResult['workflow_trace'] ?? [], 'is_array'));
    if ($workflowTrace !== []) {
        $meta['agent_trace'] = $workflowTrace;
        $meta['agent_architecture'] = 'php_supervisor_worker';
    }

    $sourceDocument = is_array($documentIngestionResult['source_document'] ?? null) ? $documentIngestionResult['source_document'] : [];
    if ($sourceDocument !== []) {
        $meta['review_status'] = aiCopilotCleanText((string) ($sourceDocument['review_status'] ?? 'pending_clinician_review'));
        $meta['source_document_id'] = isset($sourceDocument['source_document_id']) && is_numeric($sourceDocument['source_document_id'])
...
function aiCopilotAppendResponseSectionItem(array $sections, string $title, string $item, string $tone = 'yellow'): array
{
    $item = aiCopilotCleanText($item);
    if ($item === '') {
        return $sections;
    }
    ...
    foreach ($sections as $index => $section) {
        ...
        if (strtolower(trim((string) ($section['title'] ?? ''))) !== strtolower(trim($title))) {
            continue;
        }
        $items = is_array($section['items'] ?? null) ? $section['items'] : [];
        $items[] = $item;
        $sections[$index]['items'] = array_values(array_unique(array_filter(array_map('aiCopilotCleanText', $items), static fn($value) => $value !== '')));
        return $sections;
    }

    $sections[] = aiCopilotBuildSection($title, [$item], $tone);
    return $sections;
}

function aiCopilotBuildCitationContractPayload(
    array $response,
    array $context,
    string $role,
    string $mode,
    ...
    if ($toolOutput === []) {
        return [
            ...
            'validation' => [
                ...
                'source_count' => 0,
                'citation_contract_status' => 'not_applicable',
            ],
            'response' => $response,
        ];
    }

    $claims = aiCopilotBuildClaimsFromToolOutput($toolOutput);
    $trace = new AgentTrace($requestId);
    $worker = new CitationValidationWorker();
    $validation = $worker->validateClaims($claims, [
        'request_id' => $requestId,
        'patient_id' => $context['patient']['pid'] ?? ($toolOutput['document_metadata']['patient_id'] ?? null),
        'role' => $role,
    ...
    $sourcesUsed = aiCopilotBuildSourcesUsedFromClaims($validation['claims'] ?? [], $toolOutput);

    $safetyStatus = match ($validation['citation_contract_status'] ?? 'passed') {
        'blocked' => 'blocked_missing_citations',
        'review_required' => 'passed_with_clinician_review_required',
        default => 'passed_with_clinician_review_required',
    };

    $responseWithCitations = $response;
    if (($validation['blocked_claim_count'] ?? 0) > 0) {
        $responseWithCitations['sections'] = aiCopilotAppendResponseSectionItem(
            is_array($response['sections'] ?? null) ? $response['sections'] : [],
            'Missing or Ambiguous Data',
            'Some clinical claims were hidden because they were not linked to source evidence.',
            'yellow'
        );
        if (($validation['cited_claim_count'] ?? 0) <= 0) {
            $responseWithCitations['answer'] = 'I do not have enough source-grounded information to answer that safely.';
        }
    }
    if (($validation['cited_claim_count'] ?? 0) <= 0 && aiCopilotShouldMarkRagGroundedForRequest($mode, $prompt, $context)) {
        $responseWithCitations['sections'] = aiCopilotAppendResponseSectionItem(
            is_array($responseWithCitations['sections'] ?? null) ? $responseWithCitations['sections'] : [],
            'Missing or Ambiguous Data',
    ...
    return [
        'claims' => array_values(array_filter($validation['claims'] ?? [], 'is_array')),
        'sources_used' => $sourcesUsed,
        'uncited_claims_blocked' => array_values(array_filter($validation['uncited_claims_blocked'] ?? [], 'is_array')),
        'safety_status' => $safetyStatus,
        'validation' => [
            'claim_count' => (int) ($validation['claim_count'] ?? 0),
            'cited_claim_count' => (int) ($validation['cited_claim_count'] ?? 0),
            'uncited_claim_count' => (int) ($validation['uncited_claim_count'] ?? 0),
            'invalid_citation_count' => (int) ($validation['invalid_citation_count'] ?? 0),
            'blocked_claim_count' => (int) ($validation['blocked_claim_count'] ?? 0),
            'source_count' => count($sourcesUsed),
            'citation_contract_status' => (string) ($validation['citation_contract_status'] ?? 'passed'),
            'user_message' => (string) ($validation['user_message'] ?? ''),
        ],
        'response' => $responseWithCitations,
    ];
}

function aiCopilotAugmentMetaWithCitationContract(array $meta, array $citationContract): array
{
    $validation = is_array($citationContract['validation'] ?? null) ? $citationContract['validation'] : [];
    $meta['citation_contract_status'] = aiCopilotCleanText((string) ($validation['citation_contract_status'] ?? ''));
    $meta['claim_count'] = isset($validation['claim_count']) && is_numeric($validation['claim_count']) ? (int) $validation['claim_count'] : 0;
    $meta['cited_claim_count'] = isset($validation['cited_claim_count']) && is_numeric($validation['cited_claim_count']) ? (int) $validation['cited_claim_count'] : 0;
    $meta['uncited_claim_count'] = isset($validation['uncited_claim_count']) && is_numeric($validation['uncited_claim_count']) ? (int) $validation['uncited_claim_count'] : 0;
    $meta['invalid_citation_count'] = isset($validation['invalid_citation_count']) && is_numeric($validation['invalid_citation_count']) ? (int) $validation['invalid_citation_count'] : 0;
    $meta['blocked_claim_count'] = isset($validation['blocked_claim_count']) && is_numeric($validation['blocked_claim_count']) ? (int) $validation['blocked_claim_count'] : 0;
    $meta['source_count'] = isset($validation['source_count']) && is_numeric($validation['source_count']) ? (int) $validation['source_count'] : 0;
    $meta['citation_contract_user_message'] = aiCopilotCleanText((string) ($validation['user_message'] ?? ''));
    return $meta;
}
...
function aiCopilotInferModeFromMessage(string $message): string
{
    ...
    if (preg_match('/lab pdf ingestion|lab pdf|attach.*pdf|upload.*pdf|ingest.*pdf|extract.*pdf|pdf lab results?|uploaded lab evidence|uploaded evidence|attached lab evidence/', $value)) {
...
function aiCopilotBuildLlmContext(string $role, string $mode, array $context, string $message): array
{
    ...
    $attachmentReviewContext = aiCopilotIsAttachmentReviewRequest($mode, $context, $message)
        ? aiCopilotBuildAttachmentReviewContextForLlm($context)
        : [];
    ...
    $base = [
        ...
        'attachment_review_context' => $attachmentReviewContext,
    ];
    ...
    $compact = match ($mode) {
        'billing', 'billing_review' => [
            ...
            'attachment_review_context' => $base['attachment_review_context'],
        ],
        'lab_pdf_ingestion' => [
            'patient' => $base['patient'],
            'role' => $role,
...
function aiCopilotBuildAttachmentReviewContextForLlm(array $context): array
{
    $toolOutput = is_array($context['attached_lab_pdf_tool_output'] ?? null) ? $context['attached_lab_pdf_tool_output'] : [];
    if ($toolOutput === []) {
        return [
            'attachment_status' => 'no_uploaded_document_evidence',
            'instruction' => 'No uploaded document evidence is currently available for attachment review.',
        ];
    }
    ...
    if (in_array($attachmentStatus, ['invalid_file_type', 'role_blocked', 'ocr_required', 'extraction_review_required', 'document_guard_rejected', 'document_guard_review_required', 'unsupported_document', 'citation_contract_failed', 'missing_collection_date', 'missing_reference_range', 'chart_write_blocked', 'review_required'], true)) {
        return [
            ...
            'validated_uploaded_document_evidence' => false,
        ];
    }

    $sourceCoverage = is_array($toolOutput['source_coverage'] ?? null) ? $toolOutput['source_coverage'] : [];
    $documentSummaries = array_values(array_filter($toolOutput['document_summaries'] ?? [], 'is_array'));
    ...
    return aiCopilotFilterEmptyLlmValue([
        'attachment_status' => aiCopilotCleanText((string) ($toolOutput['status'] ?? 'available')),
        'requested_document_types' => array_values(array_filter(array_map('aiCopilotCleanText', $sourceCoverage['requested_document_types'] ?? []), static fn($item) => $item !== '')),
        'matched_uploaded_sources' => array_map(static function (array $source): array {
            return aiCopilotFilterEmptyLlmValue([
                'display_file_name' => aiCopilotCleanText((string) ($source['display_file_name'] ?? $source['original_file_name'] ?? '')),
                'document_type' => aiCopilotCleanText((string) ($source['document_type'] ?? $source['source_type'] ?? '')),
                'source_id' => aiCopilotCleanText((string) ($source['source_id'] ?? '')),
                'uploaded_at' => aiCopilotCleanText((string) ($source['uploaded_at'] ?? '')),
                'extraction_method' => aiCopilotCleanText((string) ($source['extraction_method'] ?? '')),
                'chunk_ids' => array_values(array_filter(array_map('aiCopilotCleanText', $source['chunk_ids'] ?? []), static fn($item) => $item !== '')),
            ]);
        }, $matchedSources),
        'missing_requested_sources_message' => aiCopilotCleanText((string) ($sourceCoverage['missing_requested_sources_message'] ?? '')),
        'document_summaries' => array_map(static function (array $summary): array {
            $labFacts = [];
            foreach (array_values(array_filter($summary['extracted_facts'] ?? [], 'is_array')) as $fact) {
                $labFacts[] = aiCopilotJoinParts([
            ...
            $intakeFields = is_array($summary['intake_fields'] ?? null) ? $summary['intake_fields'] : [];

            return aiCopilotFilterEmptyLlmValue([
                'display_file_name' => aiCopilotCleanText((string) ($summary['display_file_name'] ?? $summary['original_file_name'] ?? '')),
                'document_type' => aiCopilotCleanText((string) ($summary['document_type'] ?? '')),
                'source_id' => aiCopilotCleanText((string) ($summary['source_id'] ?? '')),
                'uploaded_at' => aiCopilotCleanText((string) ($summary['uploaded_at'] ?? '')),
                'extraction_method' => aiCopilotCleanText((string) ($summary['extraction_method'] ?? '')),
                'chunk_ids' => array_values(array_filter(array_map('aiCopilotCleanText', $summary['chunk_ids'] ?? []), static fn($item) => $item !== '')),
                'lab_facts' => array_slice(array_values(array_filter(array_map('aiCopilotCleanText', $labFacts), static fn($item) => $item !== '')), 0, 8),
                'intake_fields' => aiCopilotFilterEmptyLlmValue([
                    'reason_for_visit' => aiCopilotCleanText((string) ($intakeFields['reasonForVisit'] ?? '')),
                    'current_concerns' => aiCopilotCleanText((string) ($intakeFields['currentConcerns'] ?? '')),
                    'medication_adherence' => aiCopilotCleanText((string) ($intakeFields['medicationAdherence'] ?? '')),
                    'allergies' => aiCopilotCleanText((string) ($intakeFields['allergies'] ?? '')),
                    'insurance_update' => aiCopilotCleanText((string) ($intakeFields['insuranceUpdate'] ?? '')),
                    'care_preferences' => aiCopilotCleanText((string) ($intakeFields['carePreferences'] ?? '')),
                ]),
                'missing_data' => array_slice(array_values(array_filter(array_map('aiCopilotCleanText', $summary['missing_data'] ?? []), static fn($item) => $item !== '')), 0, 8),
            ]);
        }, $documentSummaries),
        'retrieved_text_snippets' => array_map(static function (array $chunk): array {
            $chunkText = aiCopilotCleanText((string) ($chunk['chunk_text'] ?? ''));
            if ($chunkText !== '') {
                if (function_exists('mb_substr')) {
                    $chunkText = mb_substr($chunkText, 0, 260);
                } else {
                    $chunkText = substr($chunkText, 0, 260);
                }
            }

            return aiCopilotFilterEmptyLlmValue([
                'chunk_id' => aiCopilotCleanText((string) ($chunk['id'] ?? '')),
                'display_file_name' => aiCopilotCleanText((string) ($chunk['display_file_name'] ?? $chunk['original_file_name'] ?? '')),
                'document_type' => aiCopilotCleanText((string) ($chunk['document_type'] ?? $chunk['source_type'] ?? '')),
                'chunk_text' => $chunkText,
            ]);
        }, array_slice($retrievalChunks, 0, 6)),
        'safety' => [
            'draft_only' => true,
            'review_required' => true,
            'prompt_injection_detected' => !empty($toolOutput['safety_metadata']['prompt_injection_detected']),
            'uploaded_document_priority' => true,
        ],
    ]);
}
...
function aiCopilotShouldForceLocalFallback(): bool
{
    $value = strtolower(aiCopilotReadEnv('AI_COPILOT_FORCE_LOCAL_FALLBACK'));
    return in_array($value, ['1', 'true', 'yes', 'on'], true);
}

function aiCopilotIsAttachmentReviewRequest(string $mode, array $context, string $message): bool
{
    ...
    if (!empty($context['attached_lab_pdf_tool_output'])) {
        return preg_match('/\b(uploaded|attached|attachment|pdf|lab results|lab pdf|intake form|intake)\b/i', $message) === 1;
    }

    return false;
}

function aiCopilotAttachmentReviewSourceCount(array $context): int
{
    $toolOutput = is_array($context['attached_lab_pdf_tool_output'] ?? null) ? $context['attached_lab_pdf_tool_output'] : [];
    $sourceCoverage = is_array($toolOutput['source_coverage'] ?? null) ? $toolOutput['source_coverage'] : [];
    $matchedSources = array_values(array_filter($sourceCoverage['matched_sources'] ?? [], 'is_array'));
    ...
    if ($matchedSources !== []) {
        return count($matchedSources);
    }

    return !empty($toolOutput) ? 1 : 0;
}

function aiCopilotAttachmentReviewRequestedDocumentTypes(array $context): array
{
    $toolOutput = is_array($context['attached_lab_pdf_tool_output'] ?? null) ? $context['attached_lab_pdf_tool_output'] : [];
    $sourceCoverage = is_array($toolOutput['source_coverage'] ?? null) ? $toolOutput['source_coverage'] : [];
    return array_values(array_filter(array_map('aiCopilotCleanText', $sourceCoverage['requested_document_types'] ?? []), static fn($item) => $item !== ''));
}

function aiCopilotLogAttachmentReviewLlmEvent(string $eventName, string $requestId, string $role, string $mode, array $context, array $extra = []): void
{
    $fields = array_merge([
        'request_id' => $requestId,
        'role' => $role,
        'mode' => $mode,
        'source_count' => aiCopilotAttachmentReviewSourceCount($context),
        'requested_document_types' => implode(',', aiCopilotAttachmentReviewRequestedDocumentTypes($context)),
    ], $extra);
    ...
    foreach ($fields as $key => $value) {
        ...
        $pairs[] = $key . '=' . str_replace('"', "'", aiCopilotCleanText((string) $value));
    }
...
{
    $attachmentReviewRequest = aiCopilotIsAttachmentReviewRequest($mode, $context, $message);
    if ($attachmentReviewRequest) {
        aiCopilotLogAttachmentReviewLlmEvent('copilot_llm_provider_check_started', $requestId, $role, $mode, $context, [
            'openai_configured' => aiCopilotReadEnv('OPENAI_API_KEY') !== '',
        ]);
    }

    if (aiCopilotShouldForceLocalFallback()) {
        ...
        $fallbackResponse['fallback_reason'] = 'demo_mode';
        ...
        if ($attachmentReviewRequest) {
            aiCopilotLogAttachmentReviewLlmEvent('copilot_attachment_review_fallback_used', $requestId, $role, $mode, $context, [
                'fallback_reason' => 'demo_mode',
                'openai_configured' => $fallbackResponse['openai_configured'],
            ]);
        }
    ...
    if ($openAiConfigured) {
        if ($attachmentReviewRequest) {
            aiCopilotLogAttachmentReviewLlmEvent('copilot_llm_provider_available', $requestId, $role, $mode, $context, [
                'provider' => 'openai',
            ]);
        }
        ...
        aiCopilotLogOpenAiFailure(
            ...
            aiCopilotCleanText((string) ($openAiAttempt['raw_error_message'] ?? ''))
        );
        if ($attachmentReviewRequest) {
            aiCopilotLogAttachmentReviewLlmEvent('copilot_attachment_review_llm_call_failed', $requestId, $role, $mode, $context, [
                'provider' => 'openai',
                'model' => aiCopilotCleanText((string) ($openAiAttempt['model'] ?? '')),
                'openai_error_category' => $openAiAttempt['error_category'] ?? null,
                'openai_http_status' => $openAiAttempt['http_status'] ?? null,
                'openai_error_message_safe' => $openAiAttempt['error_message_safe'] ?? null,
            ]);
        }
    } elseif ($attachmentReviewRequest) {
        aiCopilotLogAttachmentReviewLlmEvent('copilot_attachment_review_fallback_used', $requestId, $role, $mode, $context, [
    ...
    if ($attachmentReviewRequest && $openAiConfigured) {
        aiCopilotLogAttachmentReviewLlmEvent('copilot_attachment_review_fallback_used', $requestId, $role, $mode, $context, [
            'fallback_reason' => $fallbackReason,
...
function aiCopilotGenerateOpenAiDraft(string $requestId, string $apiKey, string $role, string $mode, string $message, array $chatHistory, array $context, array $modeConfig): array
{
    ...
    if ($attachmentReviewRequest) {
        aiCopilotLogAttachmentReviewLlmEvent('copilot_attachment_review_llm_prompt_built', $requestId, $role, $mode, $context, [
            'provider' => 'openai',
            'model' => $model,
        ]);
    }
    ...
    if ($attachmentReviewRequest) {
        aiCopilotLogAttachmentReviewLlmEvent('copilot_attachment_review_llm_call_started', $requestId, $role, $mode, $context, [
            'provider' => 'openai',
            'model' => $model,
        ]);
    }
    ...
    if ($responseBody === false || $curlError !== '') {
        return aiCopilotBuildOpenAiFailure('curl_error', $model, null, $curlError);
    }
    ...
    if ($httpCode >= 400) {
        return aiCopilotBuildOpenAiFailure(
            ...
            $model,
            $httpCode,
            aiCopilotExtractOpenAiErrorMessage($responseBody)
        );
    }

    $decoded = json_decode($responseBody, true);
    if (!is_array($decoded)) {
        return aiCopilotBuildOpenAiFailure('invalid_json', $model, $httpCode > 0 ? $httpCode : null, 'Unable to decode provider JSON response.');
    }
    ...
    if ($attachmentReviewRequest) {
        aiCopilotLogAttachmentReviewLlmEvent('copilot_attachment_review_llm_call_succeeded', $requestId, $role, $mode, $context, [
            'provider' => 'openai',
...
function aiCopilotBuildOpenAiFailure(string $errorCategory, string $model, ?int $httpStatus = null, string $rawErrorMessage = ''): array
{
    ...
    return [
        ...
        'model' => $model,
        'raw_error_message' => aiCopilotCleanText($rawErrorMessage),
    ];
}

function aiCopilotExtractOpenAiErrorMessage(string $responseBody): string
{
    $decoded = json_decode($responseBody, true);
    if (is_array($decoded)) {
        return aiCopilotCleanText((string) ($decoded['error']['message'] ?? ''));
    }

    return aiCopilotCleanText($responseBody);
}
...
function aiCopilotLogOpenAiFailure(string $requestId, string $role, string $mode, string $model, string $errorCategory, ?int $httpStatus = null, string $rawErrorMessage = ''): void
{
    $truncatedRawError = $rawErrorMessage;
    if ($truncatedRawError !== '') {
        if (function_exists('mb_substr')) {
            $truncatedRawError = mb_substr($truncatedRawError, 0, 280);
        } else {
            $truncatedRawError = substr($truncatedRawError, 0, 280);
        }
    }

    error_log(sprintf(
        '[OpenEMR Clinical Co-Pilot] openai_failure request_id=%s role=%s mode=%s model=%s error_category=%s http_status=%s raw_error="%s"',
        $requestId,
        $role,
        $mode,
        ...
        $truncatedRawError !== '' ? $truncatedRawError : 'n/a'
    ));
}
...
function aiCopilotBuildStructuredFormatInstruction(string $mode): string
{
    $instruction = match ($mode) {
        ...
        'lab_pdf_ingestion' => 'Return JSON only with keys: answer, sections, tags. Use sections in this order when relevant: Summary, Extracted Lab Facts or Lab Result Findings, Intake Form Findings, Missing or Ambiguous Data, Sources Used, Draft-only clinician review. Each section must contain an items array of short bullet strings grounded in the uploaded document context.',
...
function aiCopilotEnhanceDraftResponse(array $draft, string $role, string $mode, string $message, array $context): array
{
    ...
    if ($mode === 'lab_pdf_ingestion') {
        return aiCopilotEnsureAttachmentReviewResponse($draft, $context, $message);
    }
...
function aiCopilotEnsureAttachmentReviewResponse(array $draft, array $context, string $message): array
{
    $template = aiCopilotBuildLabPdfIngestionResponse($context, $message);
    $draftAnswer = aiCopilotCleanText((string) ($draft['answer'] ?? ''));
    $templateAnswer = aiCopilotCleanText((string) ($template['answer'] ?? ''));
    $sections = array_values(array_filter($draft['sections'] ?? [], 'is_array'));

    if ($draftAnswer !== '' && $draftAnswer !== $templateAnswer && !aiCopilotSectionTitleExists($sections, 'Summary')) {
        array_unshift($sections, aiCopilotBuildSection('Summary', [$draftAnswer]));
    }

    foreach (array_values(array_filter($template['sections'] ?? [], 'is_array')) as $templateSection) {
        $title = aiCopilotCleanText((string) ($templateSection['title'] ?? ''));
        if ($title === '') {
            continue;
        }

        if (in_array($title, ['Sources Used', 'Safety Notice', 'Missing or Ambiguous Data'], true)) {
            $sections = aiCopilotReplaceOrAppendSection($sections, $templateSection);
            continue;
        }

        if (!aiCopilotSectionTitleExists($sections, $title)) {
            $sections[] = $templateSection;
        }
    }

    $draft['answer'] = $templateAnswer !== '' ? $templateAnswer : ($draftAnswer !== '' ? $draftAnswer : 'Uploaded Document Ingestion — Clinician Review Required');
    $draft['sections'] = $sections;
    $draft['tags'] = aiCopilotFinalizeTags(array_merge($draft['tags'] ?? [], $template['tags'] ?? ['Review needed']));

    return $draft;
}

function aiCopilotReplaceOrAppendSection(array $sections, array $replacement): array
{
    $replacementTitle = aiCopilotCleanText((string) ($replacement['title'] ?? ''));
    if ($replacementTitle === '') {
        return $sections;
    }

    foreach ($sections as $index => $section) {
        $sectionTitle = aiCopilotCleanText((string) ($section['title'] ?? ''));
        if (strcasecmp($sectionTitle, $replacementTitle) === 0) {
...
function aiCopilotSectionTitleExists(array $sections, string $title): bool
{
    foreach ($sections as $section) {
        ...
        $sectionTitle = aiCopilotCleanText((string) ($section['title'] ?? ''));
        if ($sectionTitle !== '' && strcasecmp($sectionTitle, $title) === 0) {
            return true;
        }
    }
...
function aiCopilotAttachUploadedLabPdfContext(
    ...
    $patientId = isset($context['patient']['pid']) && is_numeric($context['patient']['pid']) ? (int) $context['patient']['pid'] : 0;
    $patientKey = aiCopilotCleanText((string) ($context['patient']['pubpid'] ?? ''));
    $patientName = aiCopilotCleanText((string) ($context['patient']['name'] ?? ''));
    $requestedDocType = aiCopilotCleanText((string) ($payload['doc_type'] ?? ''));
    $resolvedDocType = aiCopilotDocumentIngestionNormalizeDocType($requestedDocType);
    if ($resolvedDocType === 'unsupported' && $uploadedLabPdf !== null) {
        $resolvedDocType = aiCopilotDocumentIngestionNormalizeDocType(
            aiCopilotAttachmentClassifyDocumentType(
                (string) ($uploadedLabPdf['name'] ?? 'attached-document.pdf'),
                '',
                aiCopilotCleanText((string) ($payload['attachment_purpose'] ?? 'lab_pdf_ingestion'))
            )
        );
    }
    if ($resolvedDocType === 'unsupported' && $useSeededLabPdf) {
        $resolvedDocType = 'lab_pdf';
    }

    $attachmentResult = attach_and_extract(
        $patientId,
        is_array($uploadedLabPdf) ? (string) ($uploadedLabPdf['tmp_name'] ?? '') : '',
        $resolvedDocType,
        [
            'request_id' => $requestId,
            'role' => $role,
    ...
    if (is_array($attachmentResult['tool_output'] ?? null) && $attachmentResult['tool_output'] !== []) {
        $toolOutput = $attachmentResult['tool_output'];
        $toolOutput['source_links'] = is_array($attachmentResult['strict_extraction']['source_citations'] ?? null)
            ? $attachmentResult['strict_extraction']['source_citations']
            : ($toolOutput['source_links'] ?? []);
        $toolOutput['source_citations'] = is_array($attachmentResult['strict_extraction']['source_citations'] ?? null)
            ? $attachmentResult['strict_extraction']['source_citations']
            : ($toolOutput['source_citations'] ?? $toolOutput['source_links'] ?? []);
        $toolOutput['pending_review_facts'] = is_array($attachmentResult['pending_review_facts'] ?? null)
            ? $attachmentResult['pending_review_facts']
            : ($toolOutput['pending_review_facts'] ?? []);
        $toolOutput['review_queue'] = is_array($attachmentResult['review_payload'] ?? null)
            ? $attachmentResult['review_payload']
            : ($toolOutput['review_queue'] ?? []);
        $toolOutput['strict_extraction'] = is_array($attachmentResult['strict_extraction'] ?? null)
            ? $attachmentResult['strict_extraction']
            : ($toolOutput['strict_extraction'] ?? []);
        $toolOutput['schema_validation'] = is_array($attachmentResult['schema_validation'] ?? null)
            ? $attachmentResult['schema_validation']
            : ($toolOutput['schema_validation'] ?? []);
        $toolOutput['workflow_trace'] = is_array($attachmentResult['workflow_trace'] ?? null)
            ? $attachmentResult['workflow_trace']
            : ($toolOutput['workflow_trace'] ?? []);
        $toolOutput['review_payload'] = is_array($attachmentResult['review_payload'] ?? null)
            ? $attachmentResult['review_payload']
            : ($toolOutput['review_payload'] ?? []);
        if (is_array($attachmentResult['safety'] ?? null)) {
            $toolOutput['evidence_safety'] = $attachmentResult['safety'];
        }

        $context['attached_lab_pdf_tool_output'] = $toolOutput;
        $context['document_ingestion_result'] = $attachmentResult;
    }

    return $context;
}

function aiCopilotClearUploadedLabEvidenceForPatient(string $patientKey, mixed $session = null): array
{
    $patientKey = aiCopilotCleanText($patientKey);
    if ($patientKey === '') {
        return [
            'removedLabFiles' => 0,
            'removedLabChunks' => 0,
            'removedLabExtractions' => 0,
            'remainingIntakeForms' => 0,
        ];
    }

    $counts = aiCopilotLabPdfClearVectorRecords([
        'patient_key' => $patientKey,
        'source_type' => 'lab_pdf',
    ]);

    if (is_object($session) && $session instanceof \ArrayAccess) {
        foreach ([
            'attached_lab_pdf_tool_output',
            'retrieved_lab_pdf_context',
            'ai_copilot_lab_pdf_tool_output',
            'ai_copilot_retrieved_lab_pdf_context',
        ] as $key) {
    ...
    return [
        'removedLabFiles' => (int) ($counts['removedLabFiles'] ?? 0),
        'removedLabChunks' => (int) ($counts['removedLabChunks'] ?? 0),
        'removedLabExtractions' => (int) ($counts['removedLabExtractions'] ?? 0),
        'remainingIntakeForms' => (int) ($counts['remainingIntakeForms'] ?? 0),
    ];
}
...
function aiCopilotNormalizeLabPdfContext(mixed $value): array
{
    ...
    $documentType = aiCopilotCleanText((string) ($documentMetadata['documentType'] ?? $documentMetadata['document_type'] ?? $sourceMetadata['sourceType'] ?? $sourceMetadata['source_type'] ?? 'lab_pdf'));
    if ($documentType === '') {
        $documentType = 'lab_pdf';
    }
    $defaultSourceLabel = aiCopilotAttachmentSourceLabel($documentType);
    $extractedFacts = [];
    foreach (($toolOutput['extractedFacts'] ?? $toolOutput['extracted_facts'] ?? []) as $fact) {
        if (!is_array($fact)) {
            continue;
        }
        ...
        $extractedFacts[] = [
            ...
            'source_label' => aiCopilotCleanText((string) ($fact['sourceLabel'] ?? $defaultSourceLabel)),
            'source_document_id' => isset($fact['sourceDocumentId']) && is_numeric($fact['sourceDocumentId'])
                ? (int) $fact['sourceDocumentId']
                : (isset($fact['source_document_id']) && is_numeric($fact['source_document_id']) ? (int) $fact['source_document_id'] : null),
            'source_link' => is_array($fact['sourceCitation'] ?? $fact['source_citation'] ?? $fact['sourceLink'] ?? $fact['source_link'] ?? null)
                ? ($fact['sourceCitation'] ?? $fact['source_citation'] ?? $fact['sourceLink'] ?? $fact['source_link'])
                : [],
        ];
    }

    $abnormalFindings = [];
    foreach (($toolOutput['abnormalFindings'] ?? $toolOutput['abnormal_findings'] ?? []) as $item) {
        $itemText = aiCopilotCleanText((string) $item);
        if ($itemText !== '') {
            $abnormalFindings[] = $itemText;
        }
    }

    $missingData = [];
    foreach (($toolOutput['missingData'] ?? $toolOutput['missing_data'] ?? []) as $item) {
        $itemText = aiCopilotCleanText((string) $item);
        if ($itemText !== '') {
    ...
    foreach (($retrieval['chunks'] ?? []) as $chunk) {
        ...
        $retrievalChunks[] = [
            ...
            'display_file_name' => aiCopilotCleanText((string) ($chunk['displayFileName'] ?? $chunk['display_file_name'] ?? $chunk['fileName'] ?? $chunk['file_name'] ?? '')),
            'original_file_name' => aiCopilotCleanText((string) ($chunk['originalFileName'] ?? $chunk['original_file_name'] ?? $chunk['fileName'] ?? $chunk['file_name'] ?? '')),
            'source_id' => aiCopilotCleanText((string) ($chunk['sourceId'] ?? $chunk['source_id'] ?? '')),
            'document_type' => aiCopilotCleanText((string) ($chunk['documentType'] ?? $chunk['document_type'] ?? '')),
            'source_type' => aiCopilotCleanText((string) ($chunk['sourceType'] ?? $chunk['source_type'] ?? '')),
            ...
            'ingestion_origin' => aiCopilotCleanText((string) ($chunk['ingestionOrigin'] ?? $chunk['ingestion_origin'] ?? '')),
            'seeded_demo' => !empty($chunk['seededDemo']) || !empty($chunk['seeded_demo']),
    ...
    foreach (($toolOutput['vectorizedResult'] ?? $toolOutput['vectorized_result'] ?? []) as $item) {
        ...
        $vectorizedResult[] = [
            ...
            'display_file_name' => aiCopilotCleanText((string) ($item['displayFileName'] ?? $item['display_file_name'] ?? $item['fileName'] ?? $item['file_name'] ?? '')),
            'document_type' => aiCopilotCleanText((string) ($item['documentType'] ?? $item['document_type'] ?? '')),
            'source_id' => aiCopilotCleanText((string) ($item['sourceId'] ?? $item['source_id'] ?? '')),
    ...
    $intakeFields = is_array($toolOutput['intakeFields'] ?? $toolOutput['intake_fields'] ?? null)
        ? ($toolOutput['intakeFields'] ?? $toolOutput['intake_fields'])
        : [];
    $sourceCoverage = is_array($toolOutput['sourceCoverage'] ?? $toolOutput['source_coverage'] ?? null)
        ? ($toolOutput['sourceCoverage'] ?? $toolOutput['source_coverage'])
        : [];
    $documentSummaries = is_array($toolOutput['documentSummaries'] ?? $toolOutput['document_summaries'] ?? null)
        ? ($toolOutput['documentSummaries'] ?? $toolOutput['document_summaries'])
        : [];
    $documentGuard = is_array($toolOutput['documentGuard'] ?? $toolOutput['document_guard'] ?? null)
        ? ($toolOutput['documentGuard'] ?? $toolOutput['document_guard'])
        : [];
    ...
    return [
        ...
        'document_metadata' => [
            ...
            'mime_type' => aiCopilotCleanText((string) ($documentMetadata['mimeType'] ?? $documentMetadata['mime_type'] ?? 'application/pdf')),
            'size' => isset($documentMetadata['size']) && is_numeric($documentMetadata['size']) ? (int) $documentMetadata['size'] : null,
            'document_type' => $documentType,
            'patient_id' => isset($documentMetadata['patientId']) && is_numeric($documentMetadata['patientId'])
                ? (int) $documentMetadata['patientId']
                : (isset($documentMetadata['patient_id']) && is_numeric($documentMetadata['patient_id']) ? (int) $documentMetadata['patient_id'] : null),
            'seeded_demo' => !empty($documentMetadata['seededDemo']) || !empty($documentMetadata['seeded_demo']),
            'patient_key' => aiCopilotCleanText((string) ($documentMetadata['patientKey'] ?? $documentMetadata['patient_key'] ?? '')),
            'patient_name' => aiCopilotCleanText((string) ($documentMetadata['patientName'] ?? $documentMetadata['patient_name'] ?? '')),
            'uploaded_at' => aiCopilotCleanText((string) ($documentMetadata['uploadedAt'] ?? $documentMetadata['uploaded_at'] ?? '')),
            'original_file_name' => aiCopilotCleanText((string) ($documentMetadata['originalFileName'] ?? $documentMetadata['original_file_name'] ?? '')),
            'display_file_name' => aiCopilotCleanText((string) ($documentMetadata['displayFileName'] ?? $documentMetadata['display_file_name'] ?? '')),
            'source_id' => aiCopilotCleanText((string) ($documentMetadata['sourceId'] ?? $documentMetadata['source_id'] ?? '')),
            'source_document_id' => isset($documentMetadata['sourceDocumentId']) && is_numeric($documentMetadata['sourceDocumentId'])
                ? (int) $documentMetadata['sourceDocumentId']
                : (isset($documentMetadata['source_document_id']) && is_numeric($documentMetadata['source_document_id']) ? (int) $documentMetadata['source_document_id'] : null),
            'openemr_document_id' => isset($documentMetadata['openemrDocumentId']) && is_numeric($documentMetadata['openemrDocumentId'])
                ? (int) $documentMetadata['openemrDocumentId']
                : (isset($documentMetadata['openemr_document_id']) && is_numeric($documentMetadata['openemr_document_id']) ? (int) $documentMetadata['openemr_document_id'] : null),
            'fhir_document_reference_id' => aiCopilotCleanText((string) ($documentMetadata['fhirDocumentReferenceId'] ?? $documentMetadata['fhir_document_reference_id'] ?? '')),
            'fhir_binary_id' => aiCopilotCleanText((string) ($documentMetadata['fhirBinaryId'] ?? $documentMetadata['fhir_binary_id'] ?? '')),
            'review_status' => aiCopilotCleanText((string) ($documentMetadata['reviewStatus'] ?? $documentMetadata['review_status'] ?? '')),
            'file_hash' => aiCopilotCleanText((string) ($documentMetadata['fileHash'] ?? $documentMetadata['file_hash'] ?? '')),
            'uploader_role' => aiCopilotCleanText((string) ($documentMetadata['uploaderRole'] ?? $documentMetadata['uploader_role'] ?? '')),
            'uploader_user' => aiCopilotCleanText((string) ($documentMetadata['uploaderUser'] ?? $documentMetadata['uploader_user'] ?? '')),
        ],
        'source_metadata' => [
            'file_name' => aiCopilotCleanText((string) ($sourceMetadata['fileName'] ?? $sourceMetadata['file_name'] ?? '')),
            'original_file_name' => aiCopilotCleanText((string) ($sourceMetadata['originalFileName'] ?? $sourceMetadata['original_file_name'] ?? '')),
            'display_file_name' => aiCopilotCleanText((string) ($sourceMetadata['displayFileName'] ?? $sourceMetadata['display_file_name'] ?? '')),
            'uploaded_at' => aiCopilotCleanText((string) ($sourceMetadata['uploadedAt'] ?? $sourceMetadata['uploaded_at'] ?? '')),
            'source_type' => aiCopilotCleanText((string) ($sourceMetadata['sourceType'] ?? $sourceMetadata['source_type'] ?? $documentType)),
            'document_type' => aiCopilotCleanText((string) ($sourceMetadata['documentType'] ?? $sourceMetadata['document_type'] ?? $documentType)),
            'source_label' => aiCopilotCleanText((string) ($sourceMetadata['sourceLabel'] ?? $sourceMetadata['source_label'] ?? $defaultSourceLabel)),
            ...
            'source_document_id' => isset($sourceMetadata['sourceDocumentId']) && is_numeric($sourceMetadata['sourceDocumentId'])
                ? (int) $sourceMetadata['sourceDocumentId']
                : (isset($sourceMetadata['source_document_id']) && is_numeric($sourceMetadata['source_document_id']) ? (int) $sourceMetadata['source_document_id'] : null),
        ],
        ...
        'source_links' => array_values(array_filter(array_map(static function ($item): array {
            return is_array($item) ? $item : [];
        }, $toolOutput['sourceCitations'] ?? $toolOutput['source_citations'] ?? $toolOutput['sourceLinks'] ?? $toolOutput['source_links'] ?? []), static fn($item) => $item !== [])),
        'pending_review_facts' => array_values(array_filter($toolOutput['pendingReviewFacts'] ?? $toolOutput['pending_review_facts'] ?? [], 'is_array')),
        'review_queue' => is_array($toolOutput['reviewQueue'] ?? $toolOutput['review_queue'] ?? null)
            ? ($toolOutput['reviewQueue'] ?? $toolOutput['review_queue'])
            : [],
        'strict_extraction' => is_array($toolOutput['strictExtraction'] ?? $toolOutput['strict_extraction'] ?? null)
            ? ($toolOutput['strictExtraction'] ?? $toolOutput['strict_extraction'])
            : [],
        'schema_validation' => is_array($toolOutput['schemaValidation'] ?? $toolOutput['schema_validation'] ?? null)
            ? ($toolOutput['schemaValidation'] ?? $toolOutput['schema_validation'])
            : [],
        'citation_validation' => is_array($toolOutput['citationValidation'] ?? $toolOutput['citation_validation'] ?? null)
            ? ($toolOutput['citationValidation'] ?? $toolOutput['citation_validation'])
            : [],
        'safety' => [
        ...
        'document_guard' => [
            'decision' => aiCopilotCleanText((string) ($documentGuard['decision'] ?? '')),
            'document_type' => aiCopilotCleanText((string) ($documentGuard['documentType'] ?? $documentGuard['document_type'] ?? '')),
            'confidence' => isset($documentGuard['confidence']) && is_numeric($documentGuard['confidence']) ? round((float) $documentGuard['confidence'], 4) : 0.0,
            'extracted_text_preview' => aiCopilotCleanText((string) ($documentGuard['extractedTextPreview'] ?? $documentGuard['extracted_text_preview'] ?? '')),
            'detected_entity_summary' => is_array($documentGuard['detectedEntitySummary'] ?? $documentGuard['detected_entity_summary'] ?? null)
                ? ($documentGuard['detectedEntitySummary'] ?? $documentGuard['detected_entity_summary'])
                : [],
            'medical_entity_count' => isset($documentGuard['medicalEntityCount']) && is_numeric($documentGuard['medicalEntityCount'])
                ? (int) $documentGuard['medicalEntityCount']
                : (isset($documentGuard['medical_entity_count']) && is_numeric($documentGuard['medical_entity_count']) ? (int) $documentGuard['medical_entity_count'] : 0),
            'high_confidence_entity_count' => isset($documentGuard['highConfidenceEntityCount']) && is_numeric($documentGuard['highConfidenceEntityCount'])
                ? (int) $documentGuard['highConfidenceEntityCount']
                : (isset($documentGuard['high_confidence_entity_count']) && is_numeric($documentGuard['high_confidence_entity_count']) ? (int) $documentGuard['high_confidence_entity_count'] : 0),
            'rejection_reason' => aiCopilotCleanText((string) ($documentGuard['rejectionReason'] ?? $documentGuard['rejection_reason'] ?? '')),
            'guard_provider' => aiCopilotCleanText((string) ($documentGuard['guardProvider'] ?? $documentGuard['guard_provider'] ?? '')),
            'extraction_method' => aiCopilotCleanText((string) ($documentGuard['extractionMethod'] ?? $documentGuard['extraction_method'] ?? '')),
...
function aiCopilotLabPdfToolOutputForClient(array $toolOutput): array
{
    ...
    $documentType = aiCopilotCleanText((string) ($toolOutput['document_metadata']['document_type'] ?? $toolOutput['source_metadata']['source_type'] ?? 'lab_pdf'));
    if ($documentType === '') {
        $documentType = 'lab_pdf';
    }
    $defaultSourceLabel = aiCopilotAttachmentSourceLabel($documentType);
    ...
    foreach (($toolOutput['extracted_facts'] ?? []) as $fact) {
        ...
        $extractedFacts[] = [
            ...
            'interpretation' => aiCopilotCleanText((string) ($fact['interpretation'] ?? '')),
            'sourceLabel' => aiCopilotCleanText((string) ($fact['source_label'] ?? $defaultSourceLabel)),
            'sourceDocumentId' => isset($fact['source_document_id']) && is_numeric($fact['source_document_id'])
                ? (int) $fact['source_document_id']
                : null,
            'sourceLink' => is_array($fact['source_link'] ?? null) ? $fact['source_link'] : [],
        ];
    }

    $sourceDocumentId = isset($toolOutput['document_metadata']['source_document_id']) && is_numeric($toolOutput['document_metadata']['source_document_id'])
        ? (int) $toolOutput['document_metadata']['source_document_id']
        : (isset($toolOutput['source_metadata']['source_document_id']) && is_numeric($toolOutput['source_metadata']['source_document_id'])
            ? (int) $toolOutput['source_metadata']['source_document_id']
            : 0);
    if ($sourceDocumentId <= 0) {
        foreach (($toolOutput['retrieval']['chunks'] ?? []) as $chunk) {
            if (is_array($chunk) && isset($chunk['source_document_id']) && is_numeric($chunk['source_document_id'])) {
                $sourceDocumentId = (int) $chunk['source_document_id'];
                break;
            }
        }
    }

    $pendingReviewFacts = array_values(array_filter($toolOutput['pending_review_facts'] ?? [], 'is_array'));
    $reviewQueue = is_array($toolOutput['review_queue'] ?? null) ? $toolOutput['review_queue'] : [];
    if ($pendingReviewFacts === [] && $sourceDocumentId > 0 && function_exists('aiCopilotDocumentIngestionListFactsByDocument')) {
        $dbFacts = aiCopilotDocumentIngestionListFactsByDocument($sourceDocumentId);
        foreach ($dbFacts as $row) {
            if (!is_array($row)) {
                continue;
            }
            $fact = is_array($row['fact'] ?? null) ? $row['fact'] : [];
            $source = is_array($row['source'] ?? null) ? $row['source'] : [];
            $label = aiCopilotCleanText((string) (
                $fact['field_name']
                ?? $fact['test_name']
                ?? $fact['medication_name']
                ?? $fact['allergen']
                ?? $fact['relation']
                ?? 'Pending fact'
            ));
            $value = aiCopilotCleanText((string) (
                $fact['value']
                ?? $fact['normalized_value']
                ?? ''
            ));
            if ($value === '' && isset($fact['dose'], $fact['frequency'])) {
                $value = aiCopilotCleanText(trim((string) $fact['dose'] . ' ' . (string) $fact['frequency']));
            }
            if ($value === '' && isset($fact['condition'])) {
                $value = aiCopilotCleanText((string) $fact['condition']);
            }
            $pendingReviewFacts[] = [
                'id' => isset($row['id']) && is_numeric($row['id']) ? (int) $row['id'] : 0,
                'label' => $label,
                'value' => $value,
                'reviewStatus' => aiCopilotCleanText((string) ($row['review_status'] ?? 'pending_clinician_review')),
                'proposedTarget' => aiCopilotCleanText((string) ($row['proposed_target'] ?? '')),
                'confidence' => isset($row['confidence']) ? (float) $row['confidence'] : 0.0,
                'citation' => $source,
            ];
        }

        if ($reviewQueue === [] && $pendingReviewFacts !== []) {
            $summary = function_exists('aiCopilotDocumentIngestionBuildReviewSummary')
                ? aiCopilotDocumentIngestionBuildReviewSummary($sourceDocumentId)
                : [
                    'pending_count' => count(array_filter($pendingReviewFacts, static fn($fact) => ($fact['reviewStatus'] ?? '') === 'pending_clinician_review')),
                    'approved_count' => count(array_filter($pendingReviewFacts, static fn($fact) => ($fact['reviewStatus'] ?? '') === 'clinician_approved')),
                    'rejected_count' => count(array_filter($pendingReviewFacts, static fn($fact) => ($fact['reviewStatus'] ?? '') === 'clinician_rejected')),
                ];
            $reviewStatus = ($summary['pending_count'] ?? 0) > 0
                ? 'pending_clinician_review'
                : ((($summary['approved_count'] ?? 0) > 0 && ($summary['rejected_count'] ?? 0) === 0)
                    ? 'clinician_approved'
                    : ((($summary['rejected_count'] ?? 0) > 0 && ($summary['approved_count'] ?? 0) === 0)
                        ? 'clinician_rejected'
                        : 'pending_clinician_review'));
            $reviewQueue = [
                ...
                'facts' => $pendingReviewFacts,
            ];
        }
    }

    $sourceCitations = array_values(array_filter(array_map(static function ($item): array {
        return is_array($item) ? $item : [];
    }, $toolOutput['source_citations'] ?? $toolOutput['source_links'] ?? []), static fn($item) => $item !== []));
    if ($sourceCitations === [] && $pendingReviewFacts !== []) {
        $sourceCitations = array_values(array_filter(array_map(static function ($fact): array {
            return is_array($fact['citation'] ?? null) ? $fact['citation'] : [];
        }, $pendingReviewFacts), static fn($item) => $item !== []));
    }
    ...
    return [
        ...
        'documentMetadata' => [
            ...
            'patientId' => isset($toolOutput['document_metadata']['patient_id']) && is_numeric($toolOutput['document_metadata']['patient_id'])
                ? (int) $toolOutput['document_metadata']['patient_id']
                : null,
            ...
            'originalFileName' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['original_file_name'] ?? '')),
            'displayFileName' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['display_file_name'] ?? '')),
            'sourceId' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['source_id'] ?? '')),
            'sourceDocumentId' => $sourceDocumentId > 0 ? $sourceDocumentId : null,
            'openemrDocumentId' => isset($toolOutput['document_metadata']['openemr_document_id']) && is_numeric($toolOutput['document_metadata']['openemr_document_id'])
                ? (int) $toolOutput['document_metadata']['openemr_document_id']
                : null,
            'fhirDocumentReferenceId' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['fhir_document_reference_id'] ?? '')),
            'fhirBinaryId' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['fhir_binary_id'] ?? '')),
            'reviewStatus' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['review_status'] ?? '')),
            'fileHash' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['file_hash'] ?? '')),
            'uploaderRole' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['uploader_role'] ?? '')),
            'uploaderUser' => aiCopilotCleanText((string) ($toolOutput['document_metadata']['uploader_user'] ?? '')),
        ],
        'sourceMetadata' => [
            'fileName' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['file_name'] ?? '')),
            'originalFileName' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['original_file_name'] ?? '')),
            'displayFileName' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['display_file_name'] ?? '')),
            'uploadedAt' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['uploaded_at'] ?? '')),
            'sourceType' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['source_type'] ?? $documentType)),
            'documentType' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['document_type'] ?? $documentType)),
            'sourceLabel' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['source_label'] ?? $defaultSourceLabel)),
            ...
            'sourceId' => aiCopilotCleanText((string) ($toolOutput['source_metadata']['source_id'] ?? '')),
            'sourceDocumentId' => $sourceDocumentId > 0 ? $sourceDocumentId : null,
        ],
        ...
        'intakeFields' => is_array($toolOutput['intake_fields'] ?? null) ? $toolOutput['intake_fields'] : [],
        'retrieval' => [
            ...
            'chunks' => array_map(static function ($chunk): array {
                return [
                    ...
                    'displayFileName' => aiCopilotCleanText((string) ($chunk['display_file_name'] ?? $chunk['file_name'] ?? '')),
                    'originalFileName' => aiCopilotCleanText((string) ($chunk['original_file_name'] ?? $chunk['file_name'] ?? '')),
                    'sourceId' => aiCopilotCleanText((string) ($chunk['source_id'] ?? '')),
                    'documentType' => aiCopilotCleanText((string) ($chunk['document_type'] ?? '')),
                    'sourceType' => aiCopilotCleanText((string) ($chunk['source_type'] ?? '')),
                    ...
                    'ingestionOrigin' => aiCopilotCleanText((string) ($chunk['ingestion_origin'] ?? '')),
                    'sourceDocumentId' => isset($chunk['source_document_id']) && is_numeric($chunk['source_document_id'])
                        ? (int) $chunk['source_document_id']
                        : null,
                    'seededDemo' => !empty($chunk['seeded_demo']),
                    'embedding' => array_values(array_map(static fn($value) => (float) $value, is_array($chunk['embedding'] ?? null) ? $chunk['embedding'] : [])),
                ];
        ...
        'vectorizedResult' => array_map(static function ($item): array {
            return [
                ...
                'displayFileName' => aiCopilotCleanText((string) ($item['display_file_name'] ?? $item['file_name'] ?? '')),
                'documentType' => aiCopilotCleanText((string) ($item['document_type'] ?? '')),
                'sourceId' => aiCopilotCleanText((string) ($item['source_id'] ?? '')),
                'sourceDocumentId' => isset($item['source_document_id']) && is_numeric($item['source_document_id'])
        ...
        'sourceLinks' => $sourceCitations,
        'sourceCitations' => $sourceCitations,
        'pendingReviewFacts' => $pendingReviewFacts,
        'reviewQueue' => $reviewQueue,
        'strictExtraction' => is_array($toolOutput['strict_extraction'] ?? null) ? $toolOutput['strict_extraction'] : [],
        'schemaValidation' => [
            'schemaName' => aiCopilotCleanText((string) ($toolOutput['schema_validation']['schema_name'] ?? '')),
            'schemaFile' => aiCopilotCleanText((string) ($toolOutput['schema_validation']['schema_file'] ?? '')),
            'schemaValid' => !empty($toolOutput['schema_validation']['schema_valid']),
            'valid' => !empty($toolOutput['schema_validation']['schema_valid']) || !empty($toolOutput['schema_validation']['valid']),
            'reviewSafe' => !empty($toolOutput['schema_validation']['review_safe']),
            'trustedPersistenceAllowed' => !empty($toolOutput['schema_validation']['trusted_persistence_allowed']),
            'trustedRagIndexAllowed' => !empty($toolOutput['schema_validation']['trusted_rag_index_allowed']),
            'statusLabel' => aiCopilotCleanText((string) ($toolOutput['schema_validation']['status_label'] ?? '')),
            'userMessage' => aiCopilotCleanText((string) ($toolOutput['schema_validation']['user_message'] ?? '')),
            'validationErrors' => array_values(array_filter(array_map(static function ($item): array {
                return is_array($item) ? $item : [];
            }, $toolOutput['schema_validation']['validation_errors'] ?? []), static fn($item) => $item !== [])),
            'validationErrorCount' => isset($toolOutput['schema_validation']['validation_errors']) && is_array($toolOutput['schema_validation']['validation_errors'])
                ? count($toolOutput['schema_validation']['validation_errors'])
                : (isset($toolOutput['schema_validation']['validation_error_count']) && is_numeric($toolOutput['schema_validation']['validation_error_count']) ? (int) $toolOutput['schema_validation']['validation_error_count'] : 0),
            'missingRequiredFieldCount' => isset($toolOutput['schema_validation']['missing_required_field_count']) && is_numeric($toolOutput['schema_validation']['missing_required_field_count'])
                ? (int) $toolOutput['schema_validation']['missing_required_field_count']
                : 0,
            'citationCount' => isset($toolOutput['schema_validation']['citation_count']) && is_numeric($toolOutput['schema_validation']['citation_count'])
                ? (int) $toolOutput['schema_validation']['citation_count']
                : 0,
            'extractionStatus' => aiCopilotCleanText((string) ($toolOutput['schema_validation']['extraction_status'] ?? '')),
            'reviewStatus' => aiCopilotCleanText((string) ($toolOutput['schema_validation']['review_status'] ?? 'pending_clinician_review')),
            'errors' => array_values(array_filter(array_map('aiCopilotCleanText', $toolOutput['schema_validation']['errors'] ?? []), static fn($item) => $item !== '')),
        ],
        'citationValidation' => [
            'schemaName' => aiCopilotCleanText((string) ($toolOutput['citation_validation']['schema_name'] ?? 'citation_contract_v1')),
            'valid' => !empty($toolOutput['citation_validation']['valid']),
            'blocked' => !empty($toolOutput['citation_validation']['blocked']),
            'reviewRequired' => !empty($toolOutput['citation_validation']['review_required']),
            'citationContractStatus' => aiCopilotCleanText((string) ($toolOutput['citation_validation']['citation_contract_status'] ?? '')),
            'invalidCitationCount' => isset($toolOutput['citation_validation']['invalid_citation_count']) && is_numeric($toolOutput['citation_validation']['invalid_citation_count'])
                ? (int) $toolOutput['citation_validation']['invalid_citation_count']
                : 0,
            'blockedClaimCount' => isset($toolOutput['citation_validation']['blocked_claim_count']) && is_numeric($toolOutput['citation_validation']['blocked_claim_count'])
                ? (int) $toolOutput['citation_validation']['blocked_claim_count']
                : 0,
            'citationCount' => isset($toolOutput['citation_validation']['citation_count']) && is_numeric($toolOutput['citation_validation']['citation_count'])
                ? (int) $toolOutput['citation_validation']['citation_count']
                : 0,
            'validationErrors' => array_values(array_filter(array_map(static function ($item): array {
                return is_array($item) ? $item : [];
            }, $toolOutput['citation_validation']['validation_errors'] ?? []), static fn($item) => $item !== [])),
            'userMessage' => aiCopilotCleanText((string) ($toolOutput['citation_validation']['user_message'] ?? '')),
        ],
        'workflowTrace' => array_values(array_filter($toolOutput['workflow_trace'] ?? [], 'is_array')),
        'safety' => [
        ...
        'documentGuard' => [
            'decision' => aiCopilotCleanText((string) ($toolOutput['document_guard']['decision'] ?? '')),
            'documentType' => aiCopilotCleanText((string) ($toolOutput['document_guard']['document_type'] ?? '')),
            'confidence' => isset($toolOutput['document_guard']['confidence']) && is_numeric($toolOutput['document_guard']['confidence']) ? round((float) $toolOutput['document_guard']['confidence'], 4) : 0.0,
            'extractedTextPreview' => aiCopilotCleanText((string) ($toolOutput['document_guard']['extracted_text_preview'] ?? '')),
            'detectedEntitySummary' => is_array($toolOutput['document_guard']['detected_entity_summary'] ?? null) ? $toolOutput['document_guard']['detected_entity_summary'] : [],
            'medicalEntityCount' => isset($toolOutput['document_guard']['medical_entity_count']) && is_numeric($toolOutput['document_guard']['medical_entity_count']) ? (int) $toolOutput['document_guard']['medical_entity_count'] : 0,
            'highConfidenceEntityCount' => isset($toolOutput['document_guard']['high_confidence_entity_count']) && is_numeric($toolOutput['document_guard']['high_confidence_entity_count']) ? (int) $toolOutput['document_guard']['high_confidence_entity_count'] : 0,
            'rejectionReason' => aiCopilotCleanText((string) ($toolOutput['document_guard']['rejection_reason'] ?? '')),
            'guardProvider' => aiCopilotCleanText((string) ($toolOutput['document_guard']['guard_provider'] ?? '')),
            'extractionMethod' => aiCopilotCleanText((string) ($toolOutput['document_guard']['extraction_method'] ?? '')),
        ...
        'sourceCoverage' => is_array($toolOutput['source_coverage'] ?? null) ? $toolOutput['source_coverage'] : [],
        'documentSummaries' => is_array($toolOutput['document_summaries'] ?? null) ? $toolOutput['document_summaries'] : [],
        'reviewPayload' => is_array($toolOutput['review_payload'] ?? null) ? $toolOutput['review_payload'] : [],
        'evidenceSafety' => is_array($toolOutput['evidence_safety'] ?? null) ? $toolOutput['evidence_safety'] : [],
    ];
}
...
function aiCopilotAgentToolCatalog(): array
{
    return [
        ...
        'attach_and_extract' => [
            ...
            'description' => 'Store an uploaded lab PDF or intake form as an OpenEMR source document, extract draft structured facts, persist pending review facts, and return source-grounded JSON only.',
            'worker' => 'chart_retrieval_worker',
            'input_schema' => [
                'type' => 'object',
                'required' => ['role', 'patient_id', 'doc_type'],
                'properties' => [
                    'role' => ['type' => 'string'],
                    'patient_id' => ['type' => 'integer'],
                    'patient_key' => ['type' => 'string'],
                    'file_path' => ['type' => 'string'],
                    'doc_type' => ['type' => 'string', 'enum' => ['lab_pdf', 'intake_form']],
...
function aiCopilotAgentAttachAndExtractTool(array $toolInput, string $role, string $mode, string $prompt, array $context): array
{
    ...
    $sources = [];
    if ($toolOutput !== []) {
        $documentType = aiCopilotCleanText((string) ($toolOutput['source_metadata']['source_type'] ?? $toolOutput['document_metadata']['document_type'] ?? 'lab_pdf'));
        $sourceTitle = $documentType === 'intake_form' ? 'Attached Intake Form' : 'Attached Lab PDF';
        $sourceId = $documentType === 'intake_form' ? 'attached_intake_form' : 'attached_lab_pdf';
        $sources[] = aiCopilotAgentToolSource($sourceId, $sourceTitle, 'documents');
    }
...
{
    $requestId = aiCopilotCleanText((string) ($toolInput['request_id'] ?? ''));
    $trace = new AgentTrace($requestId !== '' ? $requestId : 'guideline_retrieval');
    ...
    if (in_array($mode, ['rag_chart_context', 'latest_ambient_summary', 'clinical_notes', 'treatment_plan', 'follow_up'], true)) {
    ...
    $corpusWorker = new GuidelineCorpusWorker();
    $guidelineChunks = $corpusWorker->loadChunks($requestId, $role, $mode, $trace);
    $requestedDocumentTypes = aiCopilotRagDetectRequestedDocumentTypes($prompt, $mode);
    $uploadedChunks = aiCopilotRagLoadUploadedDocumentChunks([
        'role' => $role,
        'patient_key' => aiCopilotCleanText((string) ($context['patient']['pubpid'] ?? '')),
        'requested_document_types' => $requestedDocumentTypes,
    ]);
    $allChunks = array_merge($guidelineChunks, $uploadedChunks);

    $sparseWorker = new SparseRetrievalWorker();
    $sparseResults = $sparseWorker->retrieve($allChunks, $prompt, $requestId, $role, $mode, $trace);
    $denseWorker = new DenseRetrievalWorker();
    $denseResults = $denseWorker->retrieve($allChunks, $prompt, $requestId, $role, $mode, $trace);

    $hybridWorker = new HybridRetrievalWorker();
    $hybrid = $hybridWorker->retrieve($prompt, [
        'request_id' => $requestId,
        'role' => $role,
        'mode' => $mode,
        'patient_key' => aiCopilotCleanText((string) ($context['patient']['pubpid'] ?? '')),
    ...
    $rerankWorker = new RerankWorker();
    $reranked = $rerankWorker->rerank($hybrid['candidates'] ?? [], $prompt, [
        'request_id' => $requestId,
        'role' => $role,
        'mode' => $mode,
    ], $trace);

    $retrievalMode = aiCopilotCleanText((string) ($hybrid['retrieval_mode'] ?? 'no_grounded_evidence'));
    $rerankProvider = aiCopilotCleanText((string) ($reranked['provider'] ?? 'fallback_score_sort'));
    $evidenceSnippets = [];
    foreach (($reranked['results'] ?? []) as $chunk) {
        if (!is_array($chunk)) {
            continue;
        }
        $relevanceScore = isset($chunk['relevance_score']) && is_numeric($chunk['relevance_score'])
            ? (float) $chunk['relevance_score']
            : (float) ($chunk['hybrid_score'] ?? 0.0);
        $evidenceSnippets[] = aiCopilotRagBuildEvidenceSnippet($chunk, $relevanceScore, $retrievalMode, $rerankProvider);
    }

    $evidence = [];
    $sources = [];
    $sourcesUsed = [];
    foreach ($evidenceSnippets as $snippet) {
        if (!is_array($snippet)) {
            continue;
        }
        $evidence[] = [
            'statement' => aiCopilotCleanText((string) ($snippet['text'] ?? '')),
            'source_label' => aiCopilotCleanText((string) ($snippet['source_title'] ?? 'Retrieved Evidence')),
        ];

        $citation = is_array($snippet['citation'] ?? null) ? $snippet['citation'] : [];
        if ($citation !== []) {
            $sourcesUsed[] = aiCopilotBuildSourceUsedEntry($citation, [
                'label' => aiCopilotCleanText((string) ($snippet['source_title'] ?? aiCopilotCitationSourceLabel($citation))),
            ]);
            $sources[] = [
                'id' => aiCopilotCleanText((string) ($citation['source_id'] ?? '')),
                'title' => aiCopilotCleanText((string) ($snippet['source_title'] ?? aiCopilotCitationSourceLabel($citation))),
                'label' => aiCopilotCleanText((string) ($snippet['source_title'] ?? aiCopilotCitationSourceLabel($citation))),
                'category' => aiCopilotCleanText((string) ($citation['document_type'] ?? $citation['source_type'] ?? 'source')),
            ];
        }
    }
    ...
    $missingData = [];
    if ($evidenceSnippets === []) {
        $missingData[] = 'No grounded guideline or uploaded-document evidence snippets were retrieved for this request.';
    }

    return [
        'tool' => 'retrieve_guideline_evidence',
        'worker' => 'hybrid_retrieval_worker',
        'evidence' => $evidence,
        'evidence_snippets' => $evidenceSnippets,
        'sources' => aiCopilotAgentNormalizeSources($sources),
        'sources_used' => $sourcesUsed,
        'policy_flags' => array_values(array_unique($policyFlags)),
        'retrieval_mode' => $retrievalMode !== '' ? $retrievalMode : 'no_grounded_evidence',
        'rerank_provider' => $rerankProvider !== '' ? $rerankProvider : 'fallback_score_sort',
        'guideline_chunk_count' => count($guidelineChunks),
        'uploaded_chunk_count' => count($uploadedChunks),
        'sparse_result_count' => count($sparseResults),
        'dense_result_count' => count($denseResults),
        'hybrid_candidate_count' => count($hybrid['candidates'] ?? []),
        'reranked_result_count' => count($evidenceSnippets),
...
function aiCopilotBuildNoUploadedLabEvidenceResponse(array $context): array
{
    $patientName = aiCopilotCleanText((string) ($context['patient']['name'] ?? 'Marcus Johnson'));
    $patientDisplay = $patientName !== '' ? $patientName : 'the selected demo patient';
    $message = 'No uploaded lab evidence is currently available for ' . $patientDisplay . '. Upload a lab PDF to extract and retrieve lab facts.';

    return aiCopilotBuildResponse(
        'Lab PDF Ingestion — Clinician Review Required',
        [
            aiCopilotBuildSection('Extracted Lab Facts', [$message], 'yellow'),
            aiCopilotBuildSection('Missing or Ambiguous Data', ['No uploaded lab PDF evidence is currently stored for this demo patient, so no uploaded lab facts, chunks, or retrieved evidence could be used.'], 'yellow'),
            aiCopilotBuildSection('Sources Used', ['No uploaded lab PDF sources are currently available.'], 'neutral'),
...
function aiCopilotBuildIntakeFormIngestionResponse(array $context): array
{
    $toolOutput = is_array($context['attached_lab_pdf_tool_output'] ?? null) ? $context['attached_lab_pdf_tool_output'] : [];
    ...
    $documentTitle = aiCopilotCleanText((string) ($toolOutput['document_metadata']['title'] ?? 'Attached Intake Form'));
    $intakeFields = is_array($toolOutput['intake_fields'] ?? null) ? $toolOutput['intake_fields'] : [];
    $findingItems = [];
    foreach (($toolOutput['extracted_facts'] ?? []) as $fact) {
        if (!is_array($fact)) {
        ...
        $factName = aiCopilotCleanText((string) ($fact['name'] ?? ''));
        $factValue = aiCopilotCleanText((string) ($fact['value'] ?? ''));
        $findingItems[] = $factName !== '' && $factValue !== ''
            ? $factName . ': ' . $factValue
            : aiCopilotJoinParts([
                $factName,
                $factValue,
    ...
    $missingItems = array_values(array_unique(array_filter(array_map('aiCopilotCleanText', array_merge(
        $toolOutput['missing_data'] ?? [],
        is_array($intakeFields['missingOrAmbiguousData'] ?? null) ? $intakeFields['missingOrAmbiguousData'] : []
    )), static fn($item) => $item !== '')));
    $retrievalChunks = array_values(array_filter($toolOutput['retrieval']['chunks'] ?? [], 'is_array'));
    $retrievalChunkIds = array_values(array_filter(array_map('aiCopilotCleanText', $toolOutput['retrieval']['chunk_ids'] ?? $intakeFields['sourceChunkIds'] ?? []), static fn($item) => $item !== ''));
    $uploadedAt = aiCopilotCleanText((string) ($intakeFields['uploadedTimestamp'] ?? $toolOutput['document_metadata']['uploaded_at'] ?? $toolOutput['source_metadata']['uploaded_at'] ?? ''));
    $extractionMethod = aiCopilotCleanText((string) ($toolOutput['extraction_method'] ?? ''));
    $status = aiCopilotCleanText((string) ($toolOutput['status'] ?? ''));
    $promptInjectionDetected = !empty($toolOutput['safety_metadata']['prompt_injection_detected']);

    if ($promptInjectionDetected) {
        $missingItems[] = 'Instruction-like text inside the intake form was ignored as untrusted source content and was not treated as system or workflow instructions.';
    }

    $ragNoticeItems = [
        'This draft is RAG-grounded to the uploaded intake-form chunks listed in Sources Used.',
        'The uploaded document text was treated as untrusted source content and only used as document evidence, not as instructions.',
    ];

    $sourceItems = [];
    $sourceItems[] = 'File: ' . aiCopilotFallbackValue($documentTitle, 'Uploaded intake form');
    ...
    $draftSummaryItems = [];
    $draftSummaryItems[] = 'Draft intake summary for clinician review only. Verify the extracted intake fields and any missing information directly against the original intake form before acting.';
    if (aiCopilotCleanText((string) ($intakeFields['reasonForVisit'] ?? '')) !== '') {
        $draftSummaryItems[] = 'Reason for visit identified: ' . aiCopilotCleanText((string) ($intakeFields['reasonForVisit'] ?? ''));
    }
    ...
    if ($status === 'extraction_review_required') {
        $draftSummaryItems = [
            aiCopilotCleanText((string) ($toolOutput['safe_message'] ?? 'Intake form extraction did not produce reliable intake fields. Clinician must verify the source PDF.')),
            'No structured intake facts were promoted into the draft because the extracted fields were not reliable enough for source-grounded review.',
        ];
    ...
    if ($findingItems === []) {
        $findingItems[] = $status === 'extraction_review_required'
            ? aiCopilotCleanText((string) ($toolOutput['safe_message'] ?? 'Intake form extraction did not produce reliable intake fields. Clinician must verify the source PDF.'))
            : 'No structured intake facts were extracted or retrieved from the uploaded document.';
    }

    if ($missingItems === []) {
        $missingItems[] = 'No additional missing intake-form fields were detected in the retrieved document context.';
    }
    ...
    return aiCopilotBuildResponse(
        ...
        ['Draft note', 'Chart context', 'Review needed', 'Intake form']
    );
}

function aiCopilotBuildCombinedUploadedDocumentResponse(array $context): array
{
    $toolOutput = is_array($context['attached_lab_pdf_tool_output'] ?? null) ? $context['attached_lab_pdf_tool_output'] : [];
    $patientName = aiCopilotCleanText((string) ($context['patient']['name'] ?? 'Marcus Johnson'));
    $sourceCoverage = is_array($toolOutput['source_coverage'] ?? null) ? $toolOutput['source_coverage'] : [];
    $documentSummaries = array_values(array_filter($toolOutput['document_summaries'] ?? [], 'is_array'));
    $matchedSources = array_values(array_filter($sourceCoverage['matched_sources'] ?? [], 'is_array'));
    $missingRequestedMessage = aiCopilotCleanText((string) ($sourceCoverage['missing_requested_sources_message'] ?? ''));

    $summaryItems = [];
    if ($matchedSources !== []) {
        $sourceTitles = array_values(array_unique(array_filter(array_map(static fn($source) => aiCopilotCleanText((string) ($source['display_file_name'] ?? $source['original_file_name'] ?? '')), $matchedSources), static fn($item) => $item !== '')));
        $summaryItems[] = 'Retrieved uploaded document evidence for ' . aiCopilotFallbackValue($patientName, 'the selected demo patient') . ' from: ' . aiCopilotJoinList($sourceTitles) . '.';
    } else {
        $summaryItems[] = 'No matching uploaded document evidence was retrieved for this request.';
    }
    if ($missingRequestedMessage !== '') {
        $summaryItems[] = $missingRequestedMessage;
    }
    ...
    foreach ($documentSummaries as $summary) {
        $documentType = aiCopilotCleanText((string) ($summary['document_type'] ?? ''));
        $displayFileName = aiCopilotCleanText((string) ($summary['display_file_name'] ?? $summary['original_file_name'] ?? 'Uploaded Document.pdf'));
        $chunkIds = array_values(array_filter(array_map('aiCopilotCleanText', $summary['chunk_ids'] ?? []), static fn($item) => $item !== ''));
        $extractionMethod = aiCopilotCleanText((string) ($summary['extraction_method'] ?? ''));
        $uploadedAt = aiCopilotCleanText((string) ($summary['uploaded_at'] ?? ''));

        $sourceItems[] = 'File: ' . $displayFileName
            . ' | Chunk ids: ' . ($chunkIds !== [] ? aiCopilotJoinList($chunkIds) : 'No retrieved chunk ids were available.')
            . ' | Extraction method: ' . aiCopilotFallbackValue($extractionMethod, 'not reported')
            . ' | Uploaded timestamp: ' . aiCopilotFallbackValue($uploadedAt, 'not reported');

        $summaryMissing = array_values(array_filter(array_map('aiCopilotCleanText', $summary['missing_data'] ?? []), static fn($item) => $item !== ''));
        $missingItems = array_merge($missingItems, $summaryMissing);

        if ($documentType === 'intake_form') {
            $intakeFields = is_array($summary['intake_fields'] ?? null) ? $summary['intake_fields'] : [];
            $fieldLines = [
                'Reason for visit: ' . aiCopilotFallbackValue(aiCopilotCleanText((string) ($intakeFields['reasonForVisit'] ?? '')), 'not clearly detected'),
                'Medication / adherence: ' . aiCopilotFallbackValue(aiCopilotCleanText((string) ($intakeFields['medicationAdherence'] ?? '')), 'not clearly detected'),
                'Allergies: ' . aiCopilotFallbackValue(aiCopilotCleanText((string) ($intakeFields['allergies'] ?? '')), 'not clearly detected'),
                'Insurance update: ' . aiCopilotFallbackValue(aiCopilotCleanText((string) ($intakeFields['insuranceUpdate'] ?? '')), 'not clearly detected'),
                'Care preferences: ' . aiCopilotFallbackValue(aiCopilotCleanText((string) ($intakeFields['carePreferences'] ?? '')), 'not clearly detected'),
            ];
            foreach ($fieldLines as $line) {
                $intakeItems[] = $displayFileName . ': ' . $line;
            }
            continue;
        }

        $labFacts = array_values(array_filter($summary['extracted_facts'] ?? [], 'is_array'));
        foreach ($labFacts as $fact) {
            $labItems[] = $displayFileName . ': ' . aiCopilotJoinParts([
                aiCopilotCleanText((string) ($fact['name'] ?? '')),
                aiCopilotCleanText((string) ($fact['value'] ?? '')),
                aiCopilotCleanText((string) ($fact['interpretation'] ?? '')),
            ]);
    ...
    if ($promptInjectionDetected) {
        $missingItems[] = 'Instruction-like text inside the uploaded PDFs was ignored as untrusted source content and was not treated as workflow instructions.';
    }
    if ($missingRequestedMessage !== '') {
        $missingItems[] = $missingRequestedMessage;
    }

    $requestedDocumentTypes = array_values(array_filter(array_map('aiCopilotCleanText', $sourceCoverage['requested_document_types'] ?? []), static fn($item) => $item !== ''));
    if ($labItems === [] && in_array('lab_results', $requestedDocumentTypes, true)) {
        $labItems[] = 'No uploaded lab results PDF facts were retrieved for this request.';
    }
    if ($intakeItems === [] && in_array('intake_form', $requestedDocumentTypes, true)) {
        $intakeItems[] = 'No uploaded intake-form facts were retrieved for this request.';
    }
    if ($missingItems === []) {
        $missingItems[] = 'No additional document-coverage gaps were detected in the retrieved uploaded sources.';
    }
    if ($sourceItems === []) {
        $sourceItems[] = 'No uploaded source documents were matched for this request.';
    }
    ...
    return aiCopilotBuildResponse(
        ...
        ['Draft note', 'Chart context', 'Review needed', 'Uploaded documents']
    );
}

function aiCopilotBuildLabPdfIngestionResponse(array $context, string $prompt = ''): array
{
    $toolOutput = is_array($context['attached_lab_pdf_tool_output'] ?? null) ? $context['attached_lab_pdf_tool_output'] : [];
    $documentType = aiCopilotCleanText((string) ($toolOutput['source_metadata']['source_type'] ?? $toolOutput['document_metadata']['document_type'] ?? ''));
    $sourceCoverage = is_array($toolOutput['source_coverage'] ?? null) ? $toolOutput['source_coverage'] : [];
    $documentSummaries = array_values(array_filter($toolOutput['document_summaries'] ?? [], 'is_array'));
    $requestedDocumentTypes = array_values(array_filter(array_map('aiCopilotCleanText', $sourceCoverage['requested_document_types'] ?? []), static fn($item) => $item !== ''));
    if (count($documentSummaries) > 1 || count($requestedDocumentTypes) > 1 || aiCopilotCleanText((string) ($toolOutput['source_metadata']['source_type'] ?? '')) === 'multi_document') {
        return aiCopilotBuildCombinedUploadedDocumentResponse($context);
    }
    if ($documentType === 'intake_form') {
        return aiCopilotBuildIntakeFormIngestionResponse($context);
    }

    $role = aiCopilotCleanText((string) ($context['role'] ?? 'doctor'));
    if ($toolOutput === []) {
        if (!aiCopilotLabPdfPromptExplicitlyRequestsChartContext($prompt)) {
            return aiCopilotBuildNoUploadedLabEvidenceResponse($context);
        }
...
function aiCopilotAgentDraftGroundedAnswerTool(
    ...
    $groundedWorker = new GroundedAnswerWorker();
    $workerTrace = new AgentTrace($requestId);
    $groundedDraft = $groundedWorker->draft([
        'request_id' => $requestId,
        'role' => $role,
        'mode' => $mode,
        'prompt' => $prompt,
        'context' => $context,
        'chart_context_result' => $chartContextResult,
    ...
    $meta = aiCopilotBuildResponseMeta(
        ...
        $context,
        [
            'answer' => $groundedDraft['answer'] ?? '',
            'sections' => $groundedDraft['sections'] ?? [],
            'tags' => $groundedDraft['tags'] ?? [],
        ],
        [
            'restricted_by_role' => false,
            'fallback_used' => true,
            'fallback_reason' => 'grounded_rag',
            'engine' => 'fallback',
            'provider' => 'local_fallback',
            'model' => null,
            'openai_configured' => $openAiConfigured,
            'rag_grounded' => !empty($groundedDraft['meta']['rag_grounded']),
        ],
        $requestStartedAt
    );
    $meta['retrieval_mode'] = aiCopilotCleanText((string) ($groundedDraft['meta']['retrieval_mode'] ?? $guidelineEvidenceResult['retrieval_mode'] ?? ''));
    $meta['rerank_provider'] = aiCopilotCleanText((string) ($groundedDraft['meta']['rerank_provider'] ?? $guidelineEvidenceResult['rerank_provider'] ?? ''));
    $meta['sparse_result_count'] = isset($groundedDraft['meta']['sparse_result_count']) && is_numeric($groundedDraft['meta']['sparse_result_count']) ? (int) $groundedDraft['meta']['sparse_result_count'] : (int) ($guidelineEvidenceResult['sparse_result_count'] ?? 0);
    $meta['dense_result_count'] = isset($groundedDraft['meta']['dense_result_count']) && is_numeric($groundedDraft['meta']['dense_result_count']) ? (int) $groundedDraft['meta']['dense_result_count'] : (int) ($guidelineEvidenceResult['dense_result_count'] ?? 0);
    $meta['hybrid_candidate_count'] = isset($groundedDraft['meta']['hybrid_candidate_count']) && is_numeric($groundedDraft['meta']['hybrid_candidate_count']) ? (int) $groundedDraft['meta']['hybrid_candidate_count'] : (int) ($guidelineEvidenceResult['hybrid_candidate_count'] ?? 0);
    $meta['reranked_result_count'] = isset($groundedDraft['meta']['reranked_result_count']) && is_numeric($groundedDraft['meta']['reranked_result_count']) ? (int) $groundedDraft['meta']['reranked_result_count'] : (int) ($guidelineEvidenceResult['reranked_result_count'] ?? 0);
    $meta['guideline_chunk_count'] = isset($groundedDraft['meta']['guideline_chunk_count']) && is_numeric($groundedDraft['meta']['guideline_chunk_count']) ? (int) $groundedDraft['meta']['guideline_chunk_count'] : (int) ($guidelineEvidenceResult['guideline_chunk_count'] ?? 0);
    $meta['uploaded_chunk_count'] = isset($groundedDraft['meta']['uploaded_chunk_count']) && is_numeric($groundedDraft['meta']['uploaded_chunk_count']) ? (int) $groundedDraft['meta']['uploaded_chunk_count'] : (int) ($guidelineEvidenceResult['uploaded_chunk_count'] ?? 0);
    $meta['claim_count'] = count($groundedDraft['claims'] ?? []);
    $meta['source_count'] = count($groundedDraft['sources_used'] ?? []);
    ...
    return [
        ...
        'draft' => [
            ...
            'sections' => $groundedDraft['sections'],
            'tags' => $groundedDraft['tags'],
            'sources' => $groundedDraft['sources'],
            'safety_note' => $groundedDraft['safety_note'] ?? aiCopilotGroundedAnswerSafetyNote($role),
            'missing_data' => $groundedDraft['missing_data'],
        ],
        'meta' => $meta,
        'tool_output' => $attachmentResult['tool_output'] ?? null,
        'claims' => $groundedDraft['claims'] ?? [],
        'sources_used' => $groundedDraft['sources_used'] ?? [],
        'evidence_snippets' => $groundedDraft['evidence_snippets'] ?? [],
        'uncited_claims_blocked' => [],
    ];
}
...
function aiCopilotAgentValidateCitationsTool(array $toolInput, string $role, string $mode, string $prompt, array $context): array
{
    ...
    $claims = [];
    if (is_array($toolInput['claims'] ?? null)) {
        $claims = array_values(array_filter($toolInput['claims'], 'is_array'));
    } elseif (is_array($draft['claims'] ?? null)) {
        $claims = array_values(array_filter($draft['claims'], 'is_array'));
    }

    $citationTrace = new AgentTrace($requestId !== '' ? $requestId : 'claim_citation_validation');
    $citationValidationWorker = new CitationValidationWorker();
    $claimValidation = $citationValidationWorker->validateClaims($claims, [
        'request_id' => $requestId,
        'patient_id' => isset($context['patient']['pid']) && is_numeric($context['patient']['pid']) ? (int) $context['patient']['pid'] : null,
        'role' => $role,
        'minimum_confidence' => 0.65,
        'doc_type' => aiCopilotCleanText((string) ($attachmentResult['tool_output']['document_metadata']['document_type'] ?? '')),
        'source_document_id' => aiCopilotCleanText((string) ($attachmentResult['tool_output']['source_document_id'] ?? $attachmentResult['tool_output']['document_metadata']['source_document_id'] ?? '')),
    ], $citationTrace);

    $validatedClaims = array_values(array_filter($claimValidation['claims'] ?? [], 'is_array'));
    $validatedSourcesUsed = [];
    $validatedSources = [];
    $seenSourceKeys = [];
    foreach ($validatedClaims as $claim) {
        foreach (($claim['citations'] ?? []) as $citation) {
            ...
            $sourceEntry = aiCopilotBuildSourceUsedEntry($citation, [
                'label' => aiCopilotCitationSourceLabel($citation),
            ]);
            $sourceKey = aiCopilotCleanText((string) ($sourceEntry['source_id'] ?? '')) . '|' . aiCopilotCleanText((string) ($sourceEntry['label'] ?? ''));
            if ($sourceKey === '|' || isset($seenSourceKeys[$sourceKey])) {
                continue;
            }
            $seenSourceKeys[$sourceKey] = true;
            $validatedSourcesUsed[] = $sourceEntry;
            $validatedSources[] = [
                'id' => aiCopilotCleanText((string) ($sourceEntry['source_id'] ?? '')),
                'title' => aiCopilotCleanText((string) ($sourceEntry['label'] ?? '')),
                'category' => aiCopilotCleanText((string) ($sourceEntry['document_type'] ?? $sourceEntry['source_type'] ?? 'source')),
            ];
        }
    }

    if (!empty($claimValidation['user_message'])) {
        $missingData[] = aiCopilotCleanText((string) $claimValidation['user_message']);
    }

    if ($claimValidation['claim_count'] > 0 && $validatedClaims === [] && $blockedReason === '') {
        $blockedReason = 'uncited_claim_blocked';
        $safeRefusal = aiCopilotCleanText((string) ($claimValidation['user_message'] ?? 'This response includes clinical information that could not be fully linked to source evidence. Clinician review is required before use.'));
    }
    ...
    return [
        ...
        'missing_data' => $missingData,
        'unsupported_claims' => [],
        'validated_sources' => $validatedSources !== [] ? $validatedSources : $sources,
        'validated_claims' => $validatedClaims,
        'sources_used' => $validatedSourcesUsed,
        'uncited_claims_blocked' => $claimValidation['uncited_claims_blocked'] ?? [],
        ...
        'claim_count' => (int) ($claimValidation['claim_count'] ?? 0),
        'cited_claim_count' => (int) ($claimValidation['cited_claim_count'] ?? 0),
        'uncited_claim_count' => (int) ($claimValidation['uncited_claim_count'] ?? 0),
        'invalid_citation_count' => (int) ($claimValidation['invalid_citation_count'] ?? 0),
        'blocked_claim_count' => (int) ($claimValidation['blocked_claim_count'] ?? 0),
        'citation_contract_status' => aiCopilotCleanText((string) ($claimValidation['citation_contract_status'] ?? 'passed')),
    ];
}
    ...
    if ($mode === 'lab_pdf_ingestion') {
        ...
        if (!empty($context['attached_lab_pdf_tool_output'])) {
            ...
            foreach ($matchedSources as $source) {
                $title = aiCopilotCleanText((string) ($source['display_file_name'] ?? $source['original_file_name'] ?? ''));
                if ($title !== '') {
                    $sources[] = $title;
                }
            }
            if ($sources === []) {
                $displayFileName = aiCopilotCleanText((string) ($toolOutput['source_metadata']['display_file_name'] ?? $toolOutput['document_metadata']['display_file_name'] ?? $toolOutput['document_metadata']['title'] ?? ''));
                if ($displayFileName !== '') {
                    $sources[] = $displayFileName;
                } else {
                    $documentType = aiCopilotCleanText((string) ($toolOutput['source_metadata']['source_type'] ?? $toolOutput['document_metadata']['document_type'] ?? 'lab_pdf'));
                    $sources[] = $documentType === 'intake_form' ? 'Uploaded Intake Form' : 'Uploaded Lab PDF';
                }
            }
        }
